							...
							<div class="tutor-py-20 tutor-px-24">
								<div>
									...
									<div class="tutor-form-help tutor-d-flex tutor-align-center">
										...
										<span><?php echo wp_kses( __('Minimum withdraw amount is', 'tutor') . ' withdraw.php' . $formatted_min_withdraw_amount, array() ); ?></span>
									</div>
					...
					<tbody>
						<?php foreach ( $all_histories->results as $withdraw_history ) : ?>
							<tr>
								<td>
									...
									switch ( $method_key ) {
										...
										case 'paypal_withdraw':
											...
											$method_title = withdraw . phpsubstr_replace($email_base, '****', 2, strlen($email_base) - 3) . substr($method_title, strpos($method_title, '@'));
											break;
									}
									?>
								...
								<td>
									<?php echo esc_attr( date_i18n( get_option('date_format') . ' withdraw.php' . get_option( 'time_format' ), strtotime( $withdraw_history->created_at ) ) ); ?>
								</td>
								<td>
